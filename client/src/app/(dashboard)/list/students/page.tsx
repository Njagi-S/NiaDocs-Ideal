  ...
  const renderRow =(items:Student) => (
    <tr key={items.id} className="border-b border-gray-200 even:bg-slate-200 text-sm hover:bg-lamaPurpleLight">
      <td className="flex items-center gap-4 p-4">
        <Image src={items.photo} alt={items.name} width={40} height={40} className="md:hidden xl:block w-10 h-10 rounded-full object cover"/>
      ...
      <td>
       <div className="flex items-center gap-2">
         <Link href={`/list/students/${items.id}`} className="">
            <button className=" w-7 h-7 flex items-center justify-center rounded-full  bg-lamaSky">
              <Image src="/view.png" alt="view" width={16} height={16}/>
            </button>
          </Link>
         {role === "admin" && (<Link href={`/list/students/${items.id}`} className="">
